...
def get_vocab(config):
    ...
    word_dict_file = data_dir + 'word_dict.txt'
    data_embed = data_dir + 'embed_glove_d300'
    return __get_vocab(word_dict_file,data_embed)

def get_kg_vocab(config):
    # config = json.load(open('config.py', 'r'))
    data_dir = config["redata_dir"]
    word_dict_file = data_dir + 'ent_word_dict.txt'
    data_embed = data_dir + 'ent_embed_glove_d300'
    return __get_vocab(word_dict_file,data_embed)


def __get_vocab(word_dict_file,data_embed):

    vocab = len(open(word_dict_file, 'r', encoding='utf-8').readlines())
    embed = np.zeros((vocab + 1, 300), dtype=np.float32)
    with open(data_embed) as fin:
        for line in fin:
            line = line.strip().split()
            # print(type(line[1:]))
            embed[int(line[0]) + 1] = np.array(line[1:])
    vecs = torch.FloatTensor(embed)
    del embed
    return vecs


def get_vocab_idx(config):
    # config = json.load(open('config.py', 'r'))
    data_dir = config["data_dir"]
    word_dict_file = data_dir + 'word_dict.txt'
    word_dict = {}
    for line in  open(word_dict_file).readlines():
        word,word_idx = line.rstrip().split()
        word_dict[int(word_idx)] = word
    return word_dict
...
class SimmatModule(torch.nn.Module):
    ...
    def __init__(self, single=True,padding=-1):
        super().__init__()
        self.single = single
