...
def load_embed():
    # Read Embedding File
    def read_embedding(filename,word_cnt,word_dim):
        embed = np.zeros([word_cnt+1,word_dim])
        for line in open(filename):
            line = line.strip().split()
            embed[int(line[0])+1] = list(map(float, line[1:]))
    ...
    word_cnt = len(open(data_dir+'word_dict.txt').readlines())
    vecs = read_embedding(data_embed,word_cnt,300)

    embed = torch.FloatTensor(vecs)
    embed = torch.nn.Embedding.from_pretrained(embed,padding_idx=-1)
    del vecs
    return embed
